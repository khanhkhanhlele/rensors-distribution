...
class Population:       # quần thể
    def __init__(self, path, num_available_sensors) -> None:
        ...
        self.size_of_population = self.n_scenarios*20
    ...
    def evaluate(self):
        times = np.asarray([ self.method(self.min_det_time, self.choosed_sensors(individual)) for individual in self.list_individuals])
    ...
    def next_population(self, p_cross=0.75, p_mutate=0.1):
        ...
        for child in children:
            ...
            if np.random.rand() < p_mutate:
                ...
                # print(child_mutate)
                if self.method(self.min_det_time, self.choosed_sensors(_best_individual))  >  self.method(self.min_det_time, self.choosed_sensors(child_mutate)):
                ...
                # print("un_mutate")
                # print(child)
                if self.method(self.min_det_time, self.choosed_sensors(_best_individual))  >  self.method(self.min_det_time, self.choosed_sensors(child)):
                    _next.append(child)
                else:
                    _next.append(_best_individual)
...
def genetic_algorithm(path, num_available_sensors, selectivity=0.15, n_iter=100,
    p_cross=0.75, p_mut=0.1, print_interval=100, return_history=False, verbose=False):
    pop = Population(path, num_available_sensors)
    best_sensors = pop.best_sensors
    score = float("inf")
    history = []
    for i in range(n_iter):
        pop.select(pop.size_of_population * selectivity)
        # print(pop.parents)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', required=False, help='path to network file or folder')
    time_s = time.time()
    ...
    if not os.path.exists(path): 
        ...
        best, score, history = genetic_algorithm(path = path, num_available_sensors=5, n_iter=100,
        ...
        for i, p in enumerate(os.listdir(path)):
            best, score, history = genetic_algorithm(path = os.path.join(path, p), num_available_sensors=5, n_iter=100,
            selectivity=0.15, p_cross=0.75, p_mut=0.1, print_interval=5, verbose=True, return_history=True)
            GA_time = time.time() - time_s
